...
local t = Def.ActorFrame{
    StateChangedMessageCommand=function(self,context)
        if SelectMusic.state == 1 then SetDefaultSteps() end
    end
}

function SetDefaultSteps()
    for i, pn in ipairs(GAMESTATE:GetEnabledPlayers()) do
        local index = clamp( SelectMusic.currentSteps[pn], 1, #SelectMusic.steps)
        SelectMusic.currentSteps[pn] = index
...
function StepsInputController(context)
    ...
    if context.Input == "Prev" then
        local index = SelectMusic.currentSteps[context.Player] - 1
        if index < 1 then
            index = #SelectMusic.steps
        end
        SelectMusic.currentSteps[context.Player] = index
...
function ConfirmSteps()
    ...
    if GAMESTATE:GetNumSidesJoined() > 1 then
        ...
        -- if routine then
        -- local master = GAMESTATE:GetMasterPlayer()
...
for i, pn in ipairs(GAMESTATE:GetHumanPlayers()) do
    ...
    for scroller = -numItems, numItems do
        ...
        sel[#sel+1] = Def.BitmapText{
            ...
            RefreshCommand=function(self)
                local index = scroller + SelectMusic.currentSteps[pn]
